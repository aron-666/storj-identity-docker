...
if [ $(grep -c BEGIN ~/.local/share/storj/identity/storagenode/ca.cert) -ne 2 ]; then
    echo "ca.cert is invalid"
    exit 1
fi

# check grep -c BEGIN ~/.local/share/storj/identity/storagenode/identity.cert is 3
if [ $(grep -c BEGIN ~/.local/share/storj/identity/storagenode/identity.cert) -ne 3 ]; then
